...
The grammar is **`subcommand` xor `repo` at $1**. When $1 is a subcommand (`clone`, `ls`, `shell-init`, `config`, `update`), normal cobra dispatch applies. When $1 is a repo name, the root command's `RunE` switches on `len(args)` and `args[1]`: 1 arg → bare-name hint (shell-only); 2 args, `args[1] == "where"` → resolve and print; 2 args, `args[1] == "cd"` → cd-verb hint (shell-only); 2 args, `args[1] == "w"` → tmux-window hint (shell-only); 2 args, `args[1] == "s"` → tmux-session hint (shell-only); 2 args, anything else → tool-form hint (shim-only). Cobra's `MaximumNArgs(2)` cap rejects 3+ positionals before RunE runs. The verbs `cd`, `where`, `w`, and `s` are NOT subcommands at $1 — they exist only at $2 in the repo-first form (the v0.x top-level `hop cd <name>` and `hop where <name>` were removed in the repo-verb grammar flip; `w` and `s` were never $1 subcommands).
...
| `hop <name> w [window-name]` | `root.go` (RunE 2-arg, `args[1] == "w"`) for the binary; `shell_init.go::posixInit` for the shim | 2 positionals (binary cap), 2 or 3 in the shim | Binary: prints `wHint` (`hop: 'w' is shell-only (tmux window). Add 'eval "$(hop shell-init zsh)"' to your zshrc and run inside a tmux session.`) to stderr, exits 2. Shim: `$2 == "w"` branch routes through `_hop_dispatch w "$1" "${@:3}"`, which resolves the path via `command hop "$1" where`, defaults the window name to the repo name, and invokes `tmux new-window -c "$path" -n "$name"` (no `-d` flag → tmux focuses the new window). Errors with `hop: 'w' requires an active tmux session. Use 'h <name> s' to start one.` and exits 1 when `$TMUX` is unset |
| `hop <name> s [session-name [window-name]]` | `root.go` (RunE 2-arg, `args[1] == "s"`) for the binary; `shell_init.go::posixInit` for the shim | 2 positionals (binary cap), 2-4 in the shim | Binary: prints `sHint` (`hop: 's' is shell-only (tmux session). Add 'eval "$(hop shell-init zsh)"' to your zshrc.`) to stderr, exits 2. Shim: `$2 == "s"` branch routes through `_hop_dispatch s "$1" "${@:3}"`, which resolves the path via `command hop "$1" where`, defaults both session and window names to the repo name, checks `tmux has-session -t "$session"` and errors `hop: tmux session '<name>' already exists. Use 'h <name> w' to add a window in the current session.` (exit 1) on hit. Otherwise: outside tmux invokes `tmux new-session -s "$session" -c "$path" -n "$window"` (foreground attach); inside tmux invokes `tmux new-session -d ...` then `tmux switch-client -t "$session"` (nested-tmux is forbidden) |
| `hop <name> <tool> [args...]` | `root.go` (RunE 2-arg default) for the binary; `shell_init.go::posixInit` for the shim | 2 positionals (binary cap), 2+ in the shim | Binary: prints `fmt.Sprintf(toolFormHintFmt, args[1])` (`hop: '<tool>' is not a hop verb (cd, where, w, s). For tool-form, install the shim: eval "$(hop shell-init zsh)", or use: hop -R "<name>" <tool> [args...]`) to stderr, exits 2. Shim: rewrites to `command hop -R "$1" "$2" "${@:3}"` (tool-form sugar). See [Tool-form dispatch](#tool-form-dispatch) below |
...
| `tmux` | `shell_init.go::posixInit` (`_hop_dispatch w)` / `s)` arms) | No hop-owned hint — the shim invokes `tmux new-window`, `tmux has-session`, `tmux new-session`, `tmux switch-client` directly, and a missing `tmux` binary surfaces via the shell's standard `command not found` error from the failing invocation. Lazy: only checked when the user invokes the `w` or `s` verbs through the shim. The hop binary itself remains tmux-free |
...
The root command's `RunE` returns one of five constants depending on the args shape. All five live next to `rootLong` in `src/cmd/hop/root.go` and are exit-code-2 errExitCode messages.
...
- `hop()` function with this 5-step resolution ladder (top-down, first match wins). The grammar is `subcommand` xor `repo` in `$1` — never a tool, never a verb — so `$1` has only two interpretations and the ladder needs no PATH inspection or builtin/keyword filtering:
  ...
  5. **Otherwise** — `$1` is treated as a repo name; dispatch on `$2` in this branch order: `cd`, `where`, `w`, `s`, `-R`, otherwise (with `$# == 1` short-circuiting to the `cd` arm before the chain begins):
     ...
     - **`$2 == "cd" || $2 == "where"`** → combined verb-guard branch with extra-args forwarding: if `$# > 2`, forward to the binary verbatim (`command hop "$@"`) so cobra's `MaximumNArgs(2)` raises rather than silently dropping the extras; otherwise the inner arms split: `$2 == "cd"` → `_hop_dispatch cd "$1"` (explicit `cd` verb — same dispatch as bare-name); `$2 == "where"` → `command hop "$1" where` (binary's `where`-verb dispatch handles directly).
     - **`$2 == "w"`** → `_hop_dispatch w "$1" "${@:3}"` (tmux-window verb). Unlike `cd`/`where`, the `w` branch intentionally accepts an optional 3rd positional (window name); the extra-args guard does NOT apply here.
     - **`$2 == "s"`** → `_hop_dispatch s "$1" "${@:3}"` (tmux-session verb). Like `w`, intentionally accepts optional 3rd and 4th positionals (session name, window name); no extra-args guard.
     ...
     - **otherwise (`$# >= 2`, `$2` not `cd`/`where`/`w`/`s`/`-R`)** → `command hop -R "$1" "$2" "${@:3}"` (tool-form sugar). Missing tools surface via the binary's `hop: -R: '<cmd>' not found.` error — there is no shim-side PATH check or cheerful escape hatch.

The shim does NOT call `command -v` or `type` on `$1` or `$2`, and does NOT print cheerful errors. The pre-flip ladder needed those because `$1` could be a tool, repo, or subcommand; the post-flip grammar removes the overload at the source. Direct binary invocations of the tool-form (`/path/to/hop <name> <tool>`) hit the binary's `RunE` 2-arg default branch, which prints `fmt.Sprintf(toolFormHintFmt, args[1])` and exits 2 — the tool-form is shim-only. Direct binary invocations of `hop <name>` (1 arg), `hop <name> cd`, `hop <name> w`, and `hop <name> s` similarly hit `RunE`'s respective branches and exit 2 with their hints (`bareNameHint`, `cdHint`, `wHint`, `sHint`). Only `hop <name> where` and `hop <name> -R <cmd>...` work without the shim.
- `_hop_dispatch()` helper — handles the shell-mutating `cd` path (`command hop "$2" where` then `cd --`), the URL-detected `clone` path (`cd --` to the printed path on success), the `w)` arm (resolve path, default window name to repo name, error if `$TMUX` unset, then `tmux new-window -c "$path" -n "$name"`), and the `s)` arm (resolve path, default session/window names to repo name, `tmux has-session` existence check + error-on-hit, then either `tmux new-session -s ... -c ... -n ...` (foreground attach when `$TMUX` is unset) or `tmux new-session -d ...` followed by `tmux switch-client -t ...` (when inside an existing tmux session — nested-tmux is forbidden)). The `cd)` arm has only one external call (`command hop "$2" where`); both callers (1-arg bare-name and 2-arg explicit-cd) always pass `$1` as the dispatch's `$2`, so there is no no-`$2` fallback (a previous `if [[ -z "$2" ]]; then command hop cd; fi` branch was unreachable after the case-list dropped `cd` and was removed). The `w)` and `s)` arms follow the same convention: `$2` is the repo name, `$3`/`$4` are the optional name positionals (defaulted via POSIX `${VAR:-$2}` parameter expansion).
...
| `hop outbox w` (inside tmux) | rule 5, `$2 == w` → `_hop_dispatch w "outbox"` | (no binary subcommand; `_hop_dispatch w)` invokes `command hop "outbox" where` to resolve the path, then `tmux new-window -c "$path" -n outbox` — window name defaults to repo, tmux focuses the new window) |
| `hop outbox w api` (inside tmux) | rule 5, `$2 == w` → `_hop_dispatch w "outbox" "api"` | same path resolution, then `tmux new-window -c "$path" -n api` — explicit window name `api` |
| `hop outbox w` (outside tmux) | rule 5, `$2 == w` → `_hop_dispatch w "outbox"` | `_hop_dispatch w)` checks `$TMUX` is unset, prints `hop: 'w' requires an active tmux session. Use 'h <name> s' to start one.` to stderr, returns 1 — tmux is NOT invoked |
| `hop outbox s` (outside tmux, no existing session) | rule 5, `$2 == s` → `_hop_dispatch s "outbox"` | `_hop_dispatch s)` resolves path, `tmux has-session -t outbox` returns non-zero, then `tmux new-session -s outbox -c "$path" -n outbox` (foreground attach) — session and window names both default to repo |
| `hop outbox s outbox-dev api` (inside tmux) | rule 5, `$2 == s` → `_hop_dispatch s "outbox" "outbox-dev" "api"` | resolve path, `tmux has-session -t outbox-dev` non-zero, then `tmux new-session -d -s outbox-dev -c "$path" -n api` followed by `tmux switch-client -t outbox-dev` (inside-tmux uses switch-client; nested-tmux is forbidden) |
| `hop outbox s outbox-dev` (session already exists) | rule 5, `$2 == s` → `_hop_dispatch s "outbox" "outbox-dev"` | `tmux has-session -t outbox-dev` returns 0, `_hop_dispatch s)` prints `hop: tmux session 'outbox-dev' already exists. Use 'h <name> w' to add a window in the current session.` to stderr, returns 1 — no new session, no switch-client, no attach |
...
The tool-form is **shim-only**: the binary does not interpret it. Direct binary invocations (`/path/to/hop dotfiles cursor`) hit the binary's `RunE` 2-arg default branch, which prints `fmt.Sprintf(toolFormHintFmt, "cursor")` (`hop: 'cursor' is not a hop verb (cd, where, w, s). For tool-form, install the shim: ..., or use: hop -R "<name>" <tool> [args...]`) and exits 2. Cobra's `MaximumNArgs(2)` cap rejects 3+ positionals before RunE runs (with cobra's `accepts at most 2 arg(s)` error) — including the 3-arg `w`/`s` forms (`hop outbox w api`, `hop outbox s sess win`); the shim handles those via the `${@:3}` slice, but bypassing the shim is an edge case for scripts/CI. Scripts and CI jobs that bypass the shim must use `hop -R <name> <tool>` explicitly (or `hop <name> where` for path resolution).
