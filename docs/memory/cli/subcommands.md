...
| `hop <name> open` | `open.go` (RunE 2-arg, `args[1] == "open"`) | exactly 2 positionals | Resolves `args[0]` to a path, creates a temp file, sets `WT_CD_FILE=<temp>` and `WT_WRAPPER=1` in env, execs `wt open` via `proc.RunForegroundEnv` with `cmd.Dir = repo.Path`. After wt exits, reads the temp file: if non-empty (user picked "Open here") writes path to stdout AND, when `HOP_WRAPPER` is unset, also emits `openHereNoShimHint` to stderr; otherwise emits nothing on stdout. Propagates wt's exit code via `errExitCode`; missing wt emits `hop: wt: not found on PATH.` to stderr and exits 1 (`errSilent`). Temp file is removed via `defer os.Remove`. Shim: `$2 == "open"` branch routes through `_hop_dispatch open "$1"` which captures stdout and `cd`s the parent shell iff non-empty. Replaces the v0.x tool-form `hop <name> open` (Darwin Finder via `/usr/bin/open`) — that path now reaches wt's menu instead, where "Finder" is one of the offered apps |
| `hop <name> <tool> [args...]` | `root.go` (RunE 2-arg default) for the binary; `shell_init.go::posixInit` for the shim | 2 positionals (binary cap), 2+ in the shim | Binary: prints `fmt.Sprintf(toolFormHintFmt, args[1])` (`hop: '<tool>' is not a hop verb (cd, where, open). For tool-form, install the shim: eval "$(hop shell-init zsh)", or use: hop -R "<name>" <tool> [args...]`) to stderr, exits 2. Shim: rewrites to `command hop -R "$1" "$2" "${@:3}"` (tool-form sugar). See [Tool-form dispatch](#tool-form-dispatch) below |
...
| `wt` | `open.go::runOpen` | `hop: wt: not found on PATH.` (when `wt` is missing at exec time). Mitigated: `wt` is declared as a Homebrew formula dependency (`depends_on "sahil87/tap/wt"` in `Formula/hop.rb`), so `brew install sahil87/tap/hop` pulls it automatically. The runtime hint covers non-brew installs and the rare case of manual removal |
...
## Binary-form hint texts (constants in `root.go` and `open.go`)

The root command's `RunE` returns one of three exit-code-2 errExitCode constants depending on the args shape (`bareNameHint`, `cdHint`, `toolFormHintFmt` — all in `root.go` next to `rootLong`). The `open` verb adds one more (`openHereNoShimHint` in `open.go`) — exit 0 with stderr-only output, since the path was still successfully obtained.
...
`openHereNoShimHint` (2-arg `open` verb form `hop <name> open`, when the user picks "Open here" but `HOP_WRAPPER` is unset — the binary cannot mutate the parent shell, so it prints the path to stdout AND emits this hint to stderr):

```
hop: 'Open here' requires the shell shim to cd. Add 'eval "$(hop shell-init zsh)"' to your zshrc, or use: cd "$(hop "<name>" open)"
```

Detection: `os.Getenv("HOP_WRAPPER") != "1"` — the shim's `posixInit` exports `HOP_WRAPPER=1` at top-level so the binary, invoked as a child of the shim function, sees it. Direct invocations (`/path/to/hop outbox open`) and tests don't have it set. The hint is suppressed when `HOP_WRAPPER=1`; the path is still emitted on stdout regardless (the shim's `_hop_dispatch open)` arm reads it and `cd`s the parent shell).
...
The shim also exports `HOP_WRAPPER=1` at top-level (immediately after the comment header, before the `hop()` function). The binary uses this env var to detect that it's running under the shim, suppressing the `openHereNoShimHint` for the `open` verb's "Open here" path (the shim handles the `cd` via stdout capture; the hint is for users invoking the binary directly).

The shim does NOT call `command -v` or `type` on `$1` or `$2`, and does NOT print cheerful errors. The pre-flip ladder needed those because `$1` could be a tool, repo, or subcommand; the post-flip grammar removes the overload at the source. Direct binary invocations of the tool-form (`/path/to/hop <name> <tool>`) hit the binary's `RunE` 2-arg default branch, which prints `fmt.Sprintf(toolFormHintFmt, args[1])` and exits 2 — the tool-form is shim-only. Direct binary invocations of `hop <name>` (1 arg) and `hop <name> cd` similarly hit `RunE`'s 1-arg and 2-arg-`cd` branches and exit 2 with their respective hints. `hop <name> where` and `hop <name> open` and `hop <name> -R <cmd>...` work without the shim — `where` prints the path; `open` invokes wt and emits the path on stdout for "Open here" plus the `openHereNoShimHint` to stderr (since the binary cannot mutate the parent shell directly); `-R` execs the child.
- `_hop_dispatch()` helper — handles three shell-mutating paths: the `cd` path (`command hop "$2" where` then `cd --`), the URL-detected `clone` path (`cd --` to the printed path on success), and the `open` path (`command hop "$2" open` then conditional `cd --` only if stdout is non-empty). The `cd)` arm has only one external call (`command hop "$2" where`); both callers (1-arg bare-name and 2-arg explicit-cd) always pass `$1` as the dispatch's `$2`, so there is no no-`$2` fallback. The `open)` arm mirrors `cd)` but with the `[[ -n "$target" ]]` guard since the binary intentionally prints nothing for non-"Open here" choices.
