...
| `hop <name> open` | `<name> open` | Resolve `<name>`, chdir into the path, then exec `wt open` with `WT_CD_FILE=<temp>` and `WT_WRAPPER=1` in env. wt presents its app menu; on "Open here" wt writes the path to the temp file. After wt exits, hop reads the temp file: if non-empty, prints the path to stdout (the shim's `_hop_dispatch open)` arm captures it and `cd`s the parent shell); if `HOP_WRAPPER` is unset hop also emits `openHereNoShimHint` to stderr. wt is a Homebrew formula dependency (`depends_on "sahil87/tap/wt"`). | wt's exit code on completion; 0 on success without "Open here" choice; 1 if wt is missing or resolution fails; 130 fzf cancelled |
...
The `Usage:` block enumerates (in this order): `hop`, `hop <name>`, `hop <name> cd`, `hop <name> where`, `hop <name> open`, `hop <name> -R <cmd>...`, `hop <name> <tool>...`, `hop clone <name>`, `hop clone <url>`, `hop clone --all`, `hop clone`, `hop pull <name>`, `hop pull <group>`, `hop pull --all`, `hop sync <name>`, `hop sync <group>`, `hop sync --all`, `hop ls`, `hop shell-init <shell>`, `hop config init`, `hop config where`, `hop config print`, `hop config scan <dir>`, `hop update`, `hop -h | --help`, `hop -v | --version`.
