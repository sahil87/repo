...
- [x] T010 Verify acceptance criterion #5 (`scripts/release.sh` behavior) — run each negative case (`scripts/release.sh foo`, `scripts/release.sh patch minor`, `scripts/release.sh patch` on a dirty tree) and confirm exit codes / messages. The positive-path "happy" case (`scripts/release.sh patch` on a clean tree) cannot be exercised in apply without side effects, since the script unconditionally `git push`es the new tag — verify the positive path by code-walk only, deferring the live test to release day. **Do not invoke the positive path during apply.**
